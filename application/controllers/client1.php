...
class Client extends CI_Controller {
	...
	public function escalationform(){
		...
		$data['resolution'] = $this->dropdowns->resolution_dropdown1();
	...
	public function export(){
		...
		switch($channel){
			case 'escalation':
				$this->db->select('date_received, es_from, cust_name, serial_no, brands, issues_cat, issues_details, sent_fj, block_email_fj, root_cause, resolution, cr_no, cr_deploy_date, po_cust_impact, es_status, user_fullname, last_updateby_fullname');
			...
			case 'oissues': 
				...
				$this->db->where("DATE_FORMAT(log_issues.created_date, '%Y%m') = ", $_GET['date'] );
				$this->db->where("sys_imp_id", $_GET['id'] );
				$this->db->select('log_issues.created_date,
								sys_imp, 
								log_issues.start_date, 
